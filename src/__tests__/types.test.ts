...
describe('createRequestSchema', () => {
  it('should validate valid request with threshold', () => {
    const result = createRequestSchema.safeParse({ th: '50000' });
  ...
  it('should coerce string to number', () => {
    const result = createRequestSchema.safeParse({ th: '99.5' });
    expect(result.success).toBe(true);
    if (result.success) {
  ...
  it('should reject negative threshold', () => {
    const result = createRequestSchema.safeParse({ th: '-100' });
    expect(result.success).toBe(false);
  });

  it('should reject zero threshold', () => {
    const result = createRequestSchema.safeParse({ th: '0' });
    expect(result.success).toBe(false);
  });
  ...
  it('should accept optional domain', () => {
    const result = createRequestSchema.safeParse({ th: '100', domain: 'test-domain' });
    expect(result.success).toBe(true);
    if (result.success) {
...
describe('webhookPayloadSchema', () => {
  ...
  it('should reject missing required fields', () => {
    ...
    const result = webhookPayloadSchema.safeParse(incomplete);
    expect(result.success).toBe(false);
  });
