...
function computeEventId(pubkey: string, created_at: number, kind: number, tags: string[][], content: string): string {
...
describe('sha256Hex', () => {
  it('should return a 0x-prefixed 64-char hex hash for string input', () => {
    const hash = sha256Hex('test message');
  ...
  it('should produce same hash for string and equivalent Uint8Array', () => {
    const hashStr = sha256Hex('test');
    const hashBytes = sha256Hex(new Uint8Array([116, 101, 115, 116]));
...
describe('truncateEventId', () => {
  it('should truncate to 16 chars for long IDs', () => {
    ...
    expect(truncateEventId(eventId)).toBe('abcdef1234567890');
  });

  it('should not truncate short IDs', () => {
    const shortId = 'abcd1234';
    expect(truncateEventId(shortId)).toBe('abcd1234');
  });

  it('should handle exactly 16 char IDs', () => {
    const id = '1234567890123456';
    expect(truncateEventId(id)).toBe('1234567890123456');
  });
});
...
describe('getTag', () => {
  it('should return tag value when found', () => {
    ...
    expect(getTag(tags, 'domain')).toBe('test-domain');
    expect(getTag(tags, 'type')).toBe('create');
  });

  it('should return undefined when tag not found', () => {
    const tags = [['domain', 'test']];
    expect(getTag(tags, 'nonexistent')).toBeUndefined();
  });

  it('should return undefined for empty tags', () => {
    expect(getTag([], 'domain')).toBeUndefined();
  });

  it('should return undefined for incomplete tags', () => {
    const tags = [['domain']]; // Only key, no value
    expect(getTag(tags, 'domain')).toBeUndefined();
  });
});
...
describe('generateJwt', () => {
  ...
  it('should generate a valid JWT with 3 parts', async () => {
    const jwt = await generateJwt(testPrivateKey, testAddress, testDigest, testJti);
...
describe('verifyWebhookSignature', () => {
  ...
  const testPrivateKey = 'e0144cfbe97dcb2554ebf918b1ee12c1a51d4db1385aea75ec96d6632806bb2c';
  const testPubkey = bytesToHex(schnorr.getPublicKey(testPrivateKey));
  ...
  it('should throw for invalid event_id length', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('event_id length');
  });
  ...
  it('should throw for invalid pubkey length', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('pubkey length');
  });
  ...
  it('should throw for invalid signature length', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('signature length');
  });
  ...
  it('should throw for missing event_id', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('event_id');
  });
  ...
  it('should throw for missing pubkey', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('pubkey');
  });
  ...
  it('should throw for missing signature', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('signature');
  });
  ...
  it('should throw for event_id mismatch', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('event_id mismatch');
  });
  ...
  it('should throw for all-zero signature with valid event_id', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('all-zero signature rejected');
  });
  ...
  it('should throw for all-zero pubkey with valid event_id', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('all-zero pubkey rejected');
  });
  ...
  it('should throw for invalid Schnorr signature with valid event_id', () => {
    ...
    expect(() => verifyWebhookSignature(payload)).toThrow('schnorr signature verification failed');
  });
  ...
  it('should verify valid Schnorr signature successfully', () => {
    ...
    const created_at = 1234567890;
    const kind = 1;
    const tags: string[][] = [['domain', 'test-domain']];
    const content = 'valid test content';
    const event_id = computeEventId(testPubkey, created_at, kind, tags, content);

    // Sign the event_id with Schnorr
    const eventIdBytes = sha256(new TextEncoder().encode(
      JSON.stringify([0, testPubkey, created_at, kind, tags, content])
    ));
    const signature = schnorr.sign(eventIdBytes, testPrivateKey);
    ...
    expect(() => verifyWebhookSignature(payload)).not.toThrow();
  });
});
...
describe('privateKeyToAddress', () => {
  ...
  it('should produce consistent addresses for same key', () => {
    const address1 = privateKeyToAddress(testPrivateKey);
    const address2 = privateKeyToAddress(testPrivateKey);
  ...
  it('should produce different addresses for different keys', () => {
    ...
    const address2 = privateKeyToAddress(differentKey);
...
describe('signEthereumMessage', () => {
  ...
  it('should produce a 65-byte signature', async () => {
    const signature = await signEthereumMessage(testPrivateKey, 'test message');
  ...
  it('should produce consistent signatures for same message', async () => {
    const sig1 = await signEthereumMessage(testPrivateKey, 'test');
    const sig2 = await signEthereumMessage(testPrivateKey, 'test');
    // r and s should be the same (deterministic signing)
    expect(sig1.slice(0, 64)).toEqual(sig2.slice(0, 64));
  });

  it('should produce different signatures for different messages', async () => {
    const sig1 = await signEthereumMessage(testPrivateKey, 'message1');
    const sig2 = await signEthereumMessage(testPrivateKey, 'message2');
    expect(sig1).not.toEqual(sig2);
  });

  it('should handle unicode messages correctly', async () => {
    const signature = await signEthereumMessage(testPrivateKey, 'hello 世界 🌍');
    expect(signature).toHaveLength(65);
  });
});
